...
namespace ServerModule.Modules.Auth {
using Enums;
public static partial class AuthModule {
    ...
    //     Log.Info("Auth module initialization check complete.");
    // }
    ...
    /// </summary>
    [Reducer]
    public static void AuthVerification(ReducerContext ctx, string email, bool isRegistration, string? username = null, int? roleInt = null )
    {
        ...
        if (isRegistrationAttempt)
        {
            ...
            string rVerificationCode = GenerateVerificationCode();
            Timestamp rExpiresAt = ctx.Timestamp + VerificationExpiry;
    ...
    public static void Verify(ReducerContext ctx, string verificationCode, string? deviceId)
    {
        ...
        if (user == null)
        {
            ...
            // These should have been validated and stored by AuthVerification
            if (string.IsNullOrEmpty(pending.Username) || string.IsNullOrEmpty(pending.Email))
            {
                // This indicates an inconsistent state, likely an issue in AuthVerification
    ...
    public static void UpdateProfile(ReducerContext ctx, string? newUsername, string? newEmail) // Allow updating either or both
    {
        ...
        if (!string.IsNullOrWhiteSpace(newEmail) && user.Email != newEmail)
        {
            ...
            // Could call AuthVerification internally, or instruct user to do so.
